...
TAB5_DIR="${ROOT_DIR}/platforms/tab5"
...
# Preferir el venv IDF ya instalado (evita fallo con Python 3.14 del sistema).
...
if [[ -z "${IDF_PYTHON_ENV_PATH:-}" ]]; then
  echo "Venv ESP-IDF no encontrado. Ejecuta:" >&2
  echo "  export IDF_TOOLS_PATH=... && cd \"\${IDF_PATH}\" && ./install.sh esp32p4" >&2
  exit 1
fi
